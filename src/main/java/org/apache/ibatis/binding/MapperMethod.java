...
public class MapperMethod {
    ...
    public Object execute(SqlSession sqlSession, Object[] args) {
        ...
        switch (command.getType()) {
            case INSERT: {
                ...
                /**
                 * 1、执行插入语句
                 * 2、处理返回的结果集，主要将结果转换为长整型、bool等类型
                 */
            ...
            case SELECT:
                /**
                 * 根据不同的返回类型和结果集处理器，执行不同的execute策略
                 */
    ...
    public static class SqlCommand {
        ...
        private MappedStatement resolveMappedStatement(Class<?> mapperInterface, String methodName,
            ...
            //拼接命名空间和方法名，获取statement主键
            String statementId = mapperInterface.getName() + "." + methodName;
            //检查configuration.mappedStatements容器中是否有statement
            if (configuration.hasStatement(statementId)) {
                //获取statement
                return configuration.getMappedStatement(statementId);
            } else if (mapperInterface.equals(declaringClass)) {
                //接口未申明类，同时configuration.mappedStatements中没有statement中则返回null
                return null;
            }
            /**
             *  如果configuration.mappedStatements中没有statement，
             *  但是mapperInterface存在接口，则获取父类接口再次递归解析statement
             */
